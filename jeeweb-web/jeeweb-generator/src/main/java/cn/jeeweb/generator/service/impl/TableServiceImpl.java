...
public class TableServiceImpl extends CommonServiceImpl<TableMapper, Table> implements ITableService {
    ...
    private String parsePackageNameByFunc(String packageName,String name){
        if (StringUtils.isEmpty(packageName)){
            return "";
        }
        if (!StringUtils.isEmpty(name)){
            packageName =  packageName.replace("[functionName]",name);
        }else if(packageName.startsWith("[functionName]")){
            packageName =  packageName.replace("[functionName].", "");
        }else{
            packageName =  packageName.replace(".[functionName]", "");
        }
        return packageName;
	...
	protected File getOutPath(Scheme scheme, Template template) {
        ...
        packageName = parsePackageNameByFunc(template.getTargetPackage(),scheme.getFunctionName());
		...
		String fileName ="";
		//对文件进行格式化
		if(template.getNameFormat().indexOf("[entityName]")>=0){
			fileName = template.getNameFormat().replace("[entityName]", scheme.getEntityName());
		}
		if(template.getNameFormat().indexOf("[functionName]")>=0){
			fileName = template.getNameFormat().replace("[functionName]", scheme.getFunctionName());
		}
		if(StringUtils.isEmpty(fileName)){
			fileName =template.getNameFormat();
		}
