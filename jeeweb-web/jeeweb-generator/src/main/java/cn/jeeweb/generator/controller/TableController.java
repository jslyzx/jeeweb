...
public class TableController extends BaseBeanController<Table> {
    ...
    public ModelAndView generateCode(@PathVariable("id") String id, Model model, HttpServletRequest request,
       ...
       /*if (!table.getSyncDatabase()) {
        ...
        if (scheme == null) {
            ...
            scheme.setFunctionName(StringUtils.underlineToCamel(tableName.toLowerCase()));
    ...
    @PostMapping(value = "generateCodeAll")
    public Response generateCodeAll(Queryable queryable, HttpServletRequest request) {
        String sourceId = request.getParameter("sourceId");
        EntityWrapper<Table> entityWrapper = new EntityWrapper<>(entityClass);
        entityWrapper.eq("test","0");
        entityWrapper.eq("source_id",sourceId);
        List<Table> tableList = tableService.listWithNoPage(queryable, entityWrapper);
        try {
            for(Table table: tableList){
                EntityWrapper<Scheme> schemeWrapper = new EntityWrapper<>();
                schemeWrapper.eq("table_id",table.getId());
                Scheme scheme = schemeService.selectOne(schemeWrapper);
                if (scheme != null) {
                    List<Template> templates=new ArrayList<>();
                    //所有模板
                    List<Template> allTemplates=templateService.selectList(new EntityWrapper<Template>(Template.class).eq("scheme_id",scheme.getTemplateSchemeId()));
                    ...
                    scheme.setFunctionAuthor("system");
                    scheme.setTableType(table.getTableType());
